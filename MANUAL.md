...
- Selected endpoints:
  - `POST /users`
  - `POST /posts` (runs Veil when `media_path` supplied)
  ...
  - `GET /ui`
- Optional auth: set `KNOT_API_KEY`; require `X-API-Key` on write routes.
- Redis integration:
...
- Follow style with docstrings where helpful.
